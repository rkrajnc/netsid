...
def send_INFO( sid_number ):
    ...
    conn.send( struct.pack( '!BB%ss' % len( name ), INFO, MOS_8580, name ) )
...
while True:
    ...
    try:
        ...
        start = time.clock()
        while True:
            ##print time.clock() - start, cycles / float( PHI2 )
            ...
            if paused == 1:
                ...
                if command == 0: # FLUSH
                    ...
                    ##if time.clock() - start < cycles / float( PHI2 ):
                    ##    print 'busy'
                    ##    send_BUSY()
                    ##else:
